...
// Это позволяет скачивать ROM-игры и index.html из папки public
app.use(express.static(path.join(__dirname, 'public')));

// КОНФИГУРАЦИЯ БАЗЫ ДАННЫХ И БОТОВ
const CONFIG = {
...
const UserSchema = new mongoose.Schema({
    ...
    highScores: { 
        sonic: { type: Number, default: 999999 }, tetris: { type: Number, default: 0 }, snake: { type: Number, default: 0 }, battleCity: { type: Number, default: 0 }, bomber: { type: Number, default: 0 }, gold: { type: Number, default: 0 }, roadFighter: { type: Number, default: 0 }, spaceInvaders: { type: Number, default: 0 }, airHockey: { type: Number, default: 0 }
    },
...
app.post('/api/buy-stars', async (req, res) => {
    try {
        ...
        const response = await axios.post(`https://api.telegram.org/bot${CONFIG.TELEGRAM_BOT_TOKEN}/createInvoiceLink`, {
            title: `Пополнение ${amount} ⭐️`, description: `Звезды для арены`, payload: `${telegramId}_${amount}`, provider_token: "", currency: "XTR", prices: [{ label: "Stars", amount }]
        });
